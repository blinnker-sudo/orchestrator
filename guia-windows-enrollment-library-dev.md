# Guía Windows: desarrollo local de `enrollment-library` con cambios en vivo

Esta guía documenta el proceso para tomar cambios de la lib `ib-mx-enrollment-lib` directamente en el API `ib-enrollment-journey` sin tener que publicar versiones nuevas, y reflejarlos con un solo `tsc` + reinicio de proceso, en lugar del ciclo viejo de `build:lib` + `cp -r` + reinicio.

Aplica a:
- **API**: `C:\software\journey\ib-enrollment-journey`
- **Lib MX**: `C:\software\journey\ib-mx-enrollment-lib`

Está pensada para Windows + Git Bash + WebStorm. Una guía equivalente para Mac queda pendiente.

> Nota: el nombre del folder interno de la lib (`libs/<nombre>`) puede variar. En esta guía se usa `libs/ib-mx-enrollment-lib` como placeholder; ajústalo a la ruta real que tenga el repo (puedes verlo con `ls libs/` dentro del repo de la lib).
...
```bash
# Lib
cd C:\software\journey\ib-mx-enrollment-lib
npm install

# API
cd C:\software\journey\ib-enrollment-journey
npm install
```
...
```
C:\software\journey\ib-mx-enrollment-lib\dist\libs\ib-mx-enrollment-lib\
├── index.js
├── package.json
├── journey/
└── (otros)
...
### 4. Reemplaza `node_modules\enrollment-library` con un junction al `dist/` de la lib
...
```cmd
cd /d C:\software\journey\ib-enrollment-journey
rmdir /s /q node_modules\enrollment-library
mklink /J node_modules\enrollment-library C:\software\journey\ib-mx-enrollment-lib\dist\libs\ib-mx-enrollment-lib
```

Salida esperada:

```
Vínculo de unión creado para node_modules\enrollment-library <<===>> C:\software\journey\ib-mx-enrollment-lib\dist\libs\ib-mx-enrollment-lib
```
...
```bash
cd C:\software\journey\ib-enrollment-journey
ls -la node_modules/enrollment-library
```

Debes ver:

```
lrwxrwxrwx ... enrollment-library -> /c/software/journey/ib-mx-enrollment-lib/dist/libs/ib-mx-enrollment-lib
```

Y:

```bash
cat node_modules/enrollment-library/journey/step-factory.js | head -20
```
...
2. En la Terminal A (la del watch) deberías ver:
   ...
   Found 0 errors. Watching for file changes.
   ```
3. tsc actualizó el `dist/.../<tu-archivo>.js` correspondiente. Como `node_modules\enrollment-library` del API es junction al `dist/`, los cambios ya están "ahí" para el API.
...
| `Cannot find module 'enrollment-library'` al arrancar | Junction roto | `ls -la node_modules/enrollment-library` debe mostrar la flecha al dist correcto |
...
Este comando reescribe `node_modules\enrollment-library` con la versión publicada del registry (sustituye el junction por una copia normal). Para volver a modo dev, repite los pasos 3-5 del setup inicial.
...
| 4. `mklink /J` | Conectar el `node_modules/enrollment-library` del API directo al `dist/` local de la lib, sin copiar archivos |
...
Síntoma: `ls -la` te muestra `enrollment-library ` (con espacio) en lugar de `enrollment-library`.
...
Síntoma: `cat node_modules/enrollment-library/journey/step-factory.js` dice "No such file or directory" pero `ls -la node_modules/enrollment-library` muestra el symlink.
