# Guía Windows: desarrollo local de `identity-verification-lib` con cambios en vivo

Esta guía documenta el proceso para tomar cambios de la lib `ib-mx-identity-verification-lib` directamente en el API `ib-identity-verification-journey` sin tener que publicar versiones nuevas, y reflejarlos con un solo `tsc` + reinicio de proceso, en lugar del ciclo viejo de `build:lib` + `cp -r` + reinicio.

Aplica a:
- **API**: `C:\software\journey\ib-identity-verification-journey`
- **Lib MX**: `C:\software\journey\ib-mx-identity-verification-lib`

Está pensada para Windows + Git Bash + WebStorm. Una guía equivalente para Mac queda pendiente.

> Nota: el nombre del folder interno de la lib (`libs/<nombre>`) puede variar. En esta guía se usa `libs/ib-mx-identity-verification-lib` como placeholder; ajústalo a la ruta real que tenga el repo (puedes verlo con `ls libs/` dentro del repo de la lib).
...
```bash
# Lib
cd C:\software\journey\ib-mx-identity-verification-lib
npm install

# API
cd C:\software\journey\ib-identity-verification-journey
npm install
```
...
```
C:\software\journey\ib-mx-identity-verification-lib\dist\libs\ib-mx-identity-verification-lib\
├── index.js
├── package.json
├── journey/
└── (otros)
...
### 4. Reemplaza `node_modules\identity-verification-lib` con un junction al `dist/` de la lib
...
```cmd
cd /d C:\software\journey\ib-identity-verification-journey
rmdir /s /q node_modules\identity-verification-lib
mklink /J node_modules\identity-verification-lib C:\software\journey\ib-mx-identity-verification-lib\dist\libs\ib-mx-identity-verification-lib
```

Salida esperada:

```
Vínculo de unión creado para node_modules\identity-verification-lib <<===>> C:\software\journey\ib-mx-identity-verification-lib\dist\libs\ib-mx-identity-verification-lib
```
...
```bash
cd C:\software\journey\ib-identity-verification-journey
ls -la node_modules/identity-verification-lib
```

Debes ver:

```
lrwxrwxrwx ... identity-verification-lib -> /c/software/journey/ib-mx-identity-verification-lib/dist/libs/ib-mx-identity-verification-lib
```

Y:

```bash
cat node_modules/identity-verification-lib/journey/step-factory.js | head -20
```
...
1. Editas un `.ts` en `libs/ib-mx-identity-verification-lib/src/...` y guardas.
2. En la Terminal A (la del watch) deberías ver:
   ```
   File change detected. Starting incremental compilation...
   Found 0 errors. Watching for file changes.
   ```
3. tsc actualizó el `dist/.../<tu-archivo>.js` correspondiente. Como `node_modules\identity-verification-lib` del API es junction al `dist/`, los cambios ya están "ahí" para el API.
...
| `Cannot find module 'identity-verification-lib'` al arrancar | Junction roto | `ls -la node_modules/identity-verification-lib` debe mostrar la flecha al dist correcto |
...
Este comando reescribe `node_modules\identity-verification-lib` con la versión publicada del registry (sustituye el junction por una copia normal). Para volver a modo dev, repite los pasos 3-5 del setup inicial.
...
| 4. `mklink /J` | Conectar el `node_modules/identity-verification-lib` del API directo al `dist/` local de la lib, sin copiar archivos |
...
Síntoma: `ls -la` te muestra `identity-verification-lib ` (con espacio) en lugar de `identity-verification-lib`.
...
Síntoma: `cat node_modules/identity-verification-lib/journey/step-factory.js` dice "No such file or directory" pero `ls -la node_modules/identity-verification-lib` muestra el symlink.
